...
class ViewController: UIViewController {
    ...
    var defaultsData = UserDefaults.standard    //shortcut so we don't have to type all this out
    var toDoArray = [String]()
    var toDoNotesArray = [String]()
//    var toDoArray = ["Learn Swift", "Build Apps", "Change The World!"]
//    var toDoNotesArray = ["Return clothes from Urban Outfitters", "Do all homework for the rest of the week", "Study econ for the exam on Thursday"]
    ...
    @IBAction func unwindFromDetailViewController(segue: UIStoryboardSegue) {   //either add new record or edit exisiting
...
extension ViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCellEditingStyle, forRowAt indexPath: IndexPath) {
        if editingStyle == .delete {
            toDoArray.remove(at: indexPath.row) //remove from the array itself
            toDoNotesArray.remove(at: indexPath.row)
            tableView.deleteRows(at: [indexPath], with: .fade) // remove from the cells
            saveDefaultsData()
        }
    }
